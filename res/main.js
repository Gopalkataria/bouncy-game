		...
		if (this.gameIsRunning) {
			...
			if (new_y > this.coords_allowed.top.Y && new_y < this.coords_allowed.bottom.Y) {
				...
				if (this.speed_y < 0) {
					...
					if (
						this.coords.x + 35 < Bouncer.safe_cord.max &&
						this.coords.x + 35 > Bouncer.safe_cord.min
					) {
...
class Bouncer_sprite extends Sprite {
	 ...
	 * @param {PointerEvent} e event caugth by the listener
	 */
	follow_user_phone(e) {
		try {
			let given_x = e.touches[0].clientX;
...
if (navigator.serviceWorker) {
	// checking if the browser can use it
